...
This application is provided to test the HCI UART functionality of the device. The embedded application generates packets and sends them via HCI to the ClientControl. The ClientControl app sends the test packets back. The application checks the data pattern to see whether there was any error introduced in the round trip. The application will send an error message if a packet data error is detected. The app will stop sending packets or continue after detecting an error depending on the configuration settings.
...
2. Use the Client Control tool and open the WICED HCI port. The default rate is 3000000 baud.
3. The 'Test' tab should be automatically selected.
4. Use the Test tab UI to configure the test - packet size, data pattern, number of packets, and whether to stop or continue after a packet error.
5. Use the 'Start' button to start the test.
6. Use the 'Stop' button (same button) to end the test.
7. Trace messages are sent by the app with the test results. These can be monitored with the BTSpy application.
